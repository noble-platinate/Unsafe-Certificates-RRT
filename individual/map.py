...
class Map:
    def __init__(self, height, width, step_size, start, goal):
        ...
        self.img = np.ones((self.height, self.width, 3), np.uint8) * 255
        self.img = cv2.circle(
            self.img, (self.start.x, self.start.y), 5, (0, 255, 0), -1)
        self.img = cv2.circle(
            self.img, (self.goal.x, self.goal.y), 5, (0, 0, 255), -1)
        for obstacle in self.obstacle_list:
            self.img[obstacle[0], obstacle[1]] = (0, 0, 0)
    ...
    def display_map(self,x_rand,best_path_found = False):
        ...
        if(best_path_found):
            ...
            for i in range(len(self.x_soln)-1):
                ...
                self.map = cv2.line(self.map,node_1,node_2,(0,0,255),4)
        
        for node in self.obstaclenodes:
            self.map[node.x,node.y,0]=0
            self.map[node.x,node.y,1]=0
            self.map[node.x, node.y, 2] = 0
        for node in self.freenodes:
            ...
            for i in range(len(path)-1):
                ...
                if(self.show_edges):
                    self.map = cv2.line(self.map,node_1,node_2,(255,0,0),1)
        ...
        for safety_certi in self.safety_certificates:
            ...
            cv2.circle(map1, (node_safe.x, node_safe.y),
    ...
    def normal_collision_check(self,node):
        if (self.img[node.x, node.y, 0] == 0 and self.img[node.x, node.y, 1] == 0 and self.img[node.x, node.y, 2] == 0):
            return 1
        return 0
